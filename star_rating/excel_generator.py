...
class ExcelGenerator:
    ...
    def _create_title_row(self, title: str = "星级评分报表"):
        """创建标题行（第1行）"""
...
def generate_report(
    ...
    Args:
        client: StarRatingClient 实例
        shop_ids: 门店ID列表
        platform: 平台选择，0=全平台，1=点评，2=美团
        begin_date: 开始日期，格式 YYYY-MM-DD
        end_date: 结束日期，格式 YYYY-MM-DD
        output_dir: 输出目录，默认 D:\code\meituan\star_rating\reports
    ...
    for shop_id in shop_ids:
        ...
        try:
            ...
            if result.get("code") == 200:
                ...
                print(f"    {shop_name}: 获取成功 - 点评{statistics.get('点评星级', '-')}星, 美团{statistics.get('美团星级', '-')}星")
            else:
    ...
    if is_multi_shop:
        output_file = f"{output_dir}/星级评分_多门店_{begin_date}_{end_date}.xlsx"
    else:
        output_file = f"{output_dir}/星级评分_{shop_ids[0]}_{begin_date}_{end_date}.xlsx"
