...
=== 主要作用 ===
将美团经营宝在线咨询分析的指标数据生成 Excel 汇总报表文件。
...
| 门店ID | 门店名称 | 时间 | 在线咨询人数 | ... |
|--------|---------|------|-------------|-----|
| 1933643130 | 门店A | 2026-05-01至2026-05-18 | 4 | ... |

多门店：
| 门店ID | 门店名称 | 时间 | 在线咨询人数 | ... |
|--------|---------|------|-------------|-----|
| 1933643130 | 门店A | 2026-05-01至2026-05-18 | 4 | ... |
| 57904793 | 门店B | 2026-05-01至2026-05-18 | 2 | ... |
...
class ExcelGenerator:
    ...
    def __init__(self):
        ...
        self.ws.title = "在线咨询分析报表"

    def _create_title_row(self, title: str = "在线咨询分析报表"):
        """创建标题行（第1行）"""
    ...
    def _get_widths(self):
        """获取列宽列表"""
        return [17.25, 28.875, 22.0, 18.0, 18.0, 18.0]
    ...
    def add_data_row(self, shop_id: str, shop_name: str, date_range: str, metrics: Dict):
        ...
        row_data = [
            ...
            shop_name,
            date_range,
            self._get_metric_value_float(metrics, "在线咨询人数"),
            self._get_metric_value_float(metrics, "在线咨询留资数"),
            self._get_metric_value_float(metrics, "咨询留资转化率"),
        ]
...
def generate_report(client, shop_ids: list, begin_date: str, end_date: str, platform: int = 0):
    ...
    Args:
        client: OnlineConsultationClient 实例
    ...
    if is_multi_shop:
        generator._create_title_row("在线咨询分析报表（多门店）")
    else:
    ...
    print(f"开始生成在线咨询分析报表: {begin_date} ~ {end_date}")
    ...
    if is_multi_shop:
        output_file = f"{output_dir}/在线咨询分析_多门店_{begin_date}_{end_date}.xlsx"
    else:
        output_file = f"{output_dir}/在线咨询分析_{shop_ids[0]}_{begin_date}_{end_date}.xlsx"
