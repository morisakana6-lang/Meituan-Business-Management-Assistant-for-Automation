...
| 门店ID | 门店名称 | 时间 | 花费(元) | ... |
|--------|---------|------|---------|-----|
| 764510450 | 丽减美瘦吧 | 2025-05-01至2025-05-31 | 501.66 | ... |

多门店：
| 门店ID | 门店名称 | 时间 | 花费(元) | ... |
|--------|---------|------|---------|-----|
| 675125311 | 门店A | 2025-05-01至2025-05-31 | 100.00 | ... |
| 835187138 | 门店B | 2025-05-01至2025-05-31 | 200.00 | ... |
...
class ExcelGenerator:
    ...
    def _calculate_cpc(self, cost: float, clicks: float) -> str:
        """计算点击均价 CPC"""
        if clicks > 0:
    ...
    def add_data_row(
        ...
        exposure = self._get_metric_value_float(metrics, "T30002")
        clicks = self._get_metric_value_float(metrics, "T30003")
        browse = self._get_metric_value_float(metrics, "T30005")
        view_pic = self._get_metric_value_float(metrics, "T30006")
        view_comment = self._get_metric_value_float(metrics, "T30007")
        view_info = self._get_metric_value_float(metrics, "T30039")
        view_deal = self._get_metric_value_float(metrics, "T30009")
        interested = self._get_metric_value_float(metrics, "T30083")
        group_orders = self._get_metric_value_float(metrics, "T30020")
        orders = self._get_metric_value_float(metrics, "T30049")
...
def generate_report(client, shop_ids: list, begin_date: str, end_date: str, platform: int = 0):
    ...
    if is_multi_shop:
        generator._create_title_row("推广通报表（多门店）")
    else:
    ...
    print(f"开始生成推广通报表: {begin_date} ~ {end_date}")
    ...
    if is_multi_shop:
        output_file = f"{output_dir}/推广通_多门店_{begin_date}_{end_date}.xlsx"
    else:
        output_file = f"{output_dir}/推广通_{shop_ids[0]}_{begin_date}_{end_date}.xlsx"
